...
- 👁 **预览** 对话内容，匹配关键词品红高亮，上一个/下一个跳转
- 🙈 **隐藏过程信息** 默认隐藏 thinking/tool，只看对话
- 🗑 **删除 → 回收站** 可恢复，不永久
- ♻️ **恢复** 或彻底删除
- ▶ **接续** / ⏹ **停止** / 🔄 **重启** 会话
- 🌐 **中英文切换** English & 简体中文
- 🎨 **双主题** 暖金 / 冷蓝
- 📦 **原生 macOS .app** — Launchpad、Dock 图标
...
输出 `build/S.T.O.A..app`（访达中显示为 S.T.O.A.）和 `build/S.T.O.A..dmg`。
...
- 🔍 **Search** by title, project, model, session ID, date; full-text content search with magenta highlights
- 👁 **Preview** conversation content with keyword highlighting and prev/next navigation
- 🙈 **Hide process info** — thinking/tool use hidden by default, toggle to show
- 🗑 **Delete → Trash** recoverable, not permanent
- ♻️ **Restore** or permanently delete from trash
- ▶ **Resume** / ⏹ **Stop** / 🔄 **Restart** sessions
- 🌐 **i18n** — English & 简体中文
- 🎨 **Dual theme** — Warm Gold / Cool Blue
- 📦 **Native macOS .app** — Launchpad, Dock icon
...
Outputs `build/S.T.O.A..app` (shown as S.T.O.A. in Finder) and `build/S.T.O.A..dmg`.
